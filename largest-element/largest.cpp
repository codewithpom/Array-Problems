...
int main()
{
    int arr[] = {2, 3, 2, 7, 4, 2, 7, 2}; // create a sample array
    int size = sizeof(arr) / sizeof(arr[0]); // get the size of the array
    int max_element = max(arr, size); // get the max element of the array
    std::cout << max_element << std::endl; // show the max element of the array in the console
}
